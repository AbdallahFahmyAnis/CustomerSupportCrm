...
|---|---|---|---|
| Register | `http://localhost:5000/register` | anonymous | Form: name, email, password, confirm; success → signed in at `/portal` |
...
- [ ] Automated test `[Trait("Story", "CRM-045")]` (Identity register + role force Customer)
- [ ] Mock/seed: existing `customer@crm.local` still login; register path clickable from login
- [ ] Spec, plan, and code cite `CRM-045`
