    ...
    : IRequestHandler<GetCsatReportQuery, CsatReportDto>
{
    public Task<CsatReportDto> Handle(GetCsatReportQuery request, CancellationToken cancellationToken)
    {
        ...
        var rows = db.ListFeedbackCreatedBetween(from, to);
        var count = rows.Count;
        var avg = count == 0 ? 0 : Math.Round(rows.Average(r => r.Feedback.Rating), 2);
        var distribution = Enumerable.Range(1, 5)
            .Select(rating => new CsatDistributionBucketDto(
                rating,
                rows.Count(r => r.Feedback.Rating == rating)))
            .ToList();
        var byAgent = rows
            .GroupBy(r => new { r.Ticket?.AssignedAgentId, r.Ticket?.AssignedAgentName })
            .Select(g => new CsatAgentBucketDto(
                g.Key.AssignedAgentId,
                g.Key.AssignedAgentName,
                g.Count(),
                Math.Round(g.Average(x => x.Feedback.Rating), 2)))
